...
class CyclicNetSequence(nn.Module):
    ...
    def __init__(self,
        ...
        super(CyclicNetSequence, self).__init__()
    ...
    def forward(self,
        ...
        (n, <anything>).
        """
        # x is a tensor of shape (N, L, D)
        # last_indicices is a tensor of shape (N)

        N, L, D = x.shape

        readouts = torch.zeros((N,L,2), device=x.device)
        ...
        for i in range(L):
            ...
            readouts[:,i] = self.readout(activations_sink)

        return readouts[torch.arange(N), last_indicices, :]
